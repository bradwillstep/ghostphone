...
<head>
  ...
  <title>Ultra Listener — Android Hybrid (Main + Optional Worker)</title>
  <style>
    :root{
      color-scheme: dark;
      --bg:#000;
...
<body>
  <h1>ULTRA LISTENER // ANDROID HYBRID</h1>
    ...
    <div class="card">
      <div class="row" style="justify-content: space-between; margin-bottom:10px">
        <div class="col" style="flex: 1 1 520px">
          ...
          <div class="small">Hybrid: main-thread loads by default (most compatible). Optional Worker mode can reduce stutter.</div>
        </div>
        ...
        <div class="col" style="flex: 1 1 320px">
          ...
          <div class="small">Translation requires multilingual model.</div>
        </div>
      </div>
      ...
      <div class="statusstrip">
        ...
        <span class="kv"><span class="key">MODE</span><span id="engine" class="val">MAIN</span></span>
      </div>

      <div class="row" style="margin-top:10px">
        <div class="col" style="flex:1 1 360px">
          <div class="small"><b>ENGINE</b></div>
          <label class="toggle" title="Enable Worker engine (may fail on blocked CDN).">
            <input id="workerOn" type="checkbox" />
            <span class="track"><span class="thumb"></span></span>
            <span class="label">Use Worker engine</span>
          </label>
          <div class="small">If worker is blocked, leave this OFF (MAIN is most reliable).</div>
        </div>
        ...
        <div class="col" style="flex:1 1 360px">
          <div class="small"><b>LISTENING FILTER</b> (monitor)</div>
          <div class="seg">
      ...
      <div class="row" style="margin-top:10px">
        ...
        <div class="col" style="flex:1 1 340px">
          ...
          <div class="small">Hybrid defaults to 1.5s (smoother).</div>
        </div>
      </div>

      <div class="small" style="margin-top:10px">
        Worker engine is optional. Load the model in MAIN first, then toggle Worker if you want.
      </div>
    </div>
  ...
  <script type="module">
      ...
      worker.onmessage = (ev) => {
        ...
        if (msg.type === "loaded"){
          ...
          if (msg.ok){
            ...
            showOverlay("Worker load failed", "Worker couldn't load model; switching back to MAIN.", msg.error || "unknown");
            workerOn.checked = false;
            engineEl.textContent = "MAIN";
    ...
    async function loadModel(){
      ...
      try{
        ...
        env.useBrowserCache = true;
        const t0 = performance.now();
        asrMain = await pipeline("automatic-speech-recognition", modelId, { device: "wasm" });
        const dt = performance.now() - t0;
        ...
        showOverlay("Model load failed (MAIN)", "This usually means downloads are blocked (VPN/Private DNS/adblock).", String(e?.message || e));
      }
    }
    ...
    async function runInferenceTick(){
      if (!audioCtx || !rb || !modelLoaded) return;
      ...
      if (workerOn.checked){
        ...
        if (worker){
          ...
          workerBusy = true;
          try{
            worker.postMessage({ type:"run", pcm: pcm.buffer, windowSec, task: (translateOn.checked ? "translate" : undefined) }, [pcm.buffer]);
          } catch (e){
            workerBusy = false;
          }
          return;
    ...
    btnHelp.addEventListener("click", () => {
      showOverlay(
        "Hybrid Help",
        "Load model in MAIN first (best compatibility). If MAIN stutters, enable Worker engine after loading. If Worker fails, turn it off.",
        null
      );
    });
